...
namespace ConsoleApplication1
{
    class Renamer
    {
        static void Main(string[] args)
        {
            ...
            // Create a BTVTaskListProcessor                                
            //BTVTaskListProcessor taskprocessor = new BTVTaskListProcessor(serverURL, Int32.Parse(port));
            //Console.WriteLine("pending: " + taskprocessor.GetNumberOfTasksPending(auth));
            //Console.WriteLine("process: " + taskprocessor.GetNumberOfTasksInProgress(auth));
                ...
                try
                {
                    ...
                    if (hasChannel)
                    {
                        foreach (PVSProperty pvp in mediafile.Properties)
                        {
                            ...
                            if (pvp.Name.Equals("Watched"))
                            {
                                Console.WriteLine("Watched: {0}", pvp.Value);
                                if (pvp.Value.Equals("True"))
                                    watchedFlag = "True";
                                else
                                    watchedFlag = "False";
                            }
                    ...
                    if (hasChannel && rename && !existsInMemory)
                    {
                        ...
                        else // Found multiple shows
                        {
                            ...
                            if (seriesIndex > -1)
                            {
                                ...
                                //seriesName = seriesNamesList.Item(seriesIndex).InnerText;
                                foundSeries = true;
                            }
                            else
                            {
                        ...
                        if (dateMatches <= 0) // No Matches Found, so maybe the air date was wrong
                        {
                            ...
                            if (chooseEpisode)
                            {
                                ...
                                if (episodeIndex > -1)
                                {
                                    ...
                                    //episodeTitle = episodeNameSearch[episodeIndex];
                                    foundEpisode = true;
                                }
                                else
                                {
                                ...
                                else
                                {
                                    ...
                                    if (res[0].Equals('c'))
                                    {
                                        ...
                                        for (int i = 0; i < episodes.Length; i++)
                                        {
                                            ...
                                            //episodeTitle = episodeNameSearch[episodeIndexNumber];
                                        }
                                    }
                                    else
                                        episodeIndex = -1;
                    ...
                    if (hasChannel && rename && foundSeries && seriesID != null && foundEpisode)
                    {
                        ...
                        if (!File.Exists(newfilename))
                        {
                            ...
                            {                                
                                ...
                                // Wait for library to be uploaded if changing the watched flag
                                if (watchedFlag.Equals("True"))
                                {                                    
                                    // Check if new file is in library
                                    PVSPropertyBag newfile = null;
                                    Console.WriteLine("Waiting for Library to update");
                                    while (newfile == null) {
                                        try {
                                            newfile = library.GetMediaByFullName(auth, @newfilename);
                                            Thread.Sleep(500);
                                        }
                                    ...
                                    // Maintain the Watched flag                                                                                                
                                    library.SetUserSpecificProperty(auth, @newfilename, "Watched", watchedFlag);
                                    library.EditMedia(auth, @newfilename, mediafile);
                                }                                                                                                
                            }
                        }
                        else
